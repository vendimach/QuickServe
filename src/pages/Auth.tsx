...
import { ArrowRight, CheckCircle2, Sparkles, KeyRound, Lock, Loader2, Eye, EyeOff } from "lucide-react";
...
function GoogleButton() {
  ...
  const handleClick = async () => {
    ...
    // page redirects away; busy stays true
  };
  return (
    <button
      type="button"
...
export default function Auth() {
  ...
  // Already authenticated — about to navigate, render nothing to avoid flash
  if (user) return null;
...
function SignInForm() {
  ...
  const signInWithPassword = async (e: React.FormEvent) => {
    ...
    // On success, Auth's useEffect handles the redirect
  };
        ...
        {!otpSent ? (
          ...
          <form onSubmit={verifyOtp} className="space-y-3">
            ...
            <div>
              ...
              <Input id="otp-code" inputMode="numeric" maxLength={6} value={otp}
                onChange={(e) => { setOtp(e.target.value); setError(""); }} placeholder="123456" />
            </div>
            {error && <p className="rounded-lg bg-destructive/10 px-3 py-2 text-xs font-medium text-destructive">{error}</p>}
            <Button type="submit" className="w-full" disabled={loading}>
    ...
    <form onSubmit={signInWithPassword} className="space-y-3">
      ...
      <div>
        ...
        <div className="relative">
          <Input id="si-pwd" type={showPwd ? "text" : "password"} required value={password}
...
// ─── Sign-up (creates auth account only — profile/role are set in onboarding) ─
function SignUpFlow({ onSwitchToSignIn }: { onSwitchToSignIn: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!data.session) {
      // Email confirmation required — Supabase project has email confirm ON
      setNeedsConfirmation(true);
      return;
    }
    // If session exists, onAuthStateChange fires and Auth's useEffect redirects to /onboarding
  };

  if (needsConfirmation) {
    return (
      <div className="text-center py-8 animate-fade-in-up space-y-3">
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-center text-muted-foreground">
        Create your account — we'll set up your profile next
      </p>
      <form onSubmit={submit} className="space-y-3">
        ...
        <div>
          ...
          <div className="relative">
            <Input id="su-pwd" type={showPwd ? "text" : "password"} required minLength={6} value={password}
              onChange={(e) => { setPassword(e.target.value); setError(""); }} className="pr-10" />
            <button type="button" tabIndex={-1} onClick={() => setShowPwd((v) => !v)}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
              {showPwd ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
        ...
        <div>
          ...
          <Input id="su-cpwd" type="password" required value={confirmPassword}
            onChange={(e) => { setConfirmPassword(e.target.value); setError(""); }} />
        </div>
        {error && <p className="rounded-lg bg-destructive/10 px-3 py-2 text-xs font-medium text-destructive">{error}</p>}
        <Button type="submit" className="w-full" disabled={loading}>
          {loading ? <><Loader2 className="h-4 w-4 animate-spin mr-1.5" />Creating account…</> : <>Create Account <ArrowRight className="h-4 w-4 ml-1" /></>}
        </Button>
      </form>
      <Divider />
      <GoogleButton />
